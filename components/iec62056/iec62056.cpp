...
void IEC62056Component::setup() {
  ...
  if (force_mode_d_) {
    ESP_LOGW(TAG, "Mode D is not supported in this version, ignoring.");
    // Do not set Mode D states
    force_mode_d_ = false;
  }
  if (is_periodic_readout_enabled_()) {
    // Start the first readout after 15s as in original
    wait_(15000, BEGIN);
  } else {
...
void IEC62056Component::parse_id_(const char *packet) {
  ...
  // Ignore meter-provided baud rate, use fixed 9600 ('5')
  baud_rate_identification_ = '5';
  ESP_LOGVV(TAG, "Baudrate char (ignored actual, using '5'): '%c'", baud_rate_identification_);
...
void IEC62056Component::loop() {
  ...
  switch (state_) {
    ...
    case PREPARE_ACK:
      ...
      // wait longer for meter to be ready before switching baud rate
      wait_(1500, SET_BAUD_RATE);
      break;
...
void IEC62056Component::verify_all_sensors_got_value_() {
  for (const auto &item : sensors_) {
    ...
    if (!s->has_value()) {
      ESP_LOGE(TAG,
               "Not all sensors received data from the meter. The first one: OBIS '%s'. Verify sensor is defined with valid OBIS code.",
               s->get_obis().c_str());
